...
MVP Stage 1~3은 폰 세로 화면을 11x20 그리드로 보고 한 화면에 모두 표시한다. x=5를 화면 중앙으로 두며, 맵 스크롤은 사용하지 않는다.
...
각 스테이지의 첫 매대 해금 비용은 기본 5 Gold로 시작한다. 유저는 스테이지 시작 Gold 5를 사용해 첫 매대를 열고, 이후 판매 수익으로 레벨업과 추가 매대를 진행한다. 이 시작 Gold는 이후 전역 업그레이드로 늘릴 수 있는 후보로 둔다.
...
첫 손님은 첫 상품을 주문해 첫 매대 해금칸 점멸을 유도한다. 두 번째 이후 상품은 해금되지 않은 상태에서 주문 대상이 되지 않는다. 주문 가능한 상품이 늘어나면 상품 선택 확률은 열린 상품 수 기준으로 다시 균등 분배된다. 손님 +1 업그레이드 전에는 `one-in-one-out`으로 처리한다. 손님 1명이 입장, 주문, 상품 수령, 퇴장을 완료한 뒤 다음 손님이 등장한다. 상품을 기다리는 동안의 대기는 줄서기/대기열이 아니라 현재 주문 손님이 매장 안에 머무르는 상태다.
