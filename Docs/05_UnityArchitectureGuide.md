...
1. `WorkAssignmentService`가 주문을 상품 1개 단위 작업으로 조회
2. 앞선 주문부터 확인하되 생산 슬롯이 예약/점유 중인 작업은 이번 배정에서 건너뜀
3. 현재 생산 가능한 작업을 찾으면 즉시 생산 슬롯을 예약
4. 에이전트는 예약된 슬롯 주변의 최단 작업 위치로 이동
5. 생산 완료 후 슬롯 예약을 해제하고 해당 손님에게 상품 전달
